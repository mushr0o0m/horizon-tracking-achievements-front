...
const LEVELS: AchievementLevel[] = ['Международный', 'Всероссийский', 'Региональный', 'Вузовский', 'Факультетский'];
...
export function EventForm({ initialEvent, onBack, onSave }: EventFormProps) {
  ...
  const [level, setLevel] = useState<AchievementLevel>(initialEvent?.level ?? 'Региональный');
  const [type, setType] = useState<EventType>(initialEvent?.type ?? 'Олимпиада');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...
      status: initialEvent?.status ?? 'Черновик',
    });
  };
  ...
  return (
    <div className="flex flex-col gap-6 max-w-xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">
            {initialEvent ? 'Внесите изменения и сохраните' : 'Заполните данные нового мероприятия'}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-card border border-border rounded-xl p-6 space-y-5">
        {/* Title */}
        <div>
          <label className={labelClass}>Название <span className="text-destructive">*</span></label>
          <input
            type="text"
            value={title}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className={labelClass}>Уровень <span className="text-destructive">*</span></label>
            <select
              value={level}
          ...
          <div>
            <label className={labelClass}>Тип <span className="text-destructive">*</span></label>
            <select
              value={type}
        ...
        {/* Date */}
        <div>
          <label className={labelClass}>Дата <span className="text-destructive">*</span></label>
          <input
            type="date"
            value={date}
