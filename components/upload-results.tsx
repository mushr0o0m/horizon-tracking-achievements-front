...
const MOCK_PARTICIPANTS: Omit<Participant, 'result'>[] = [
  { id: 'p-1', studentId: CURRENT_STUDENT.id, studentName: CURRENT_STUDENT.name },
  { id: 'p-2', studentId: 'student-2', studentName: 'Петрова Мария Ивановна' },
  { id: 'p-3', studentId: 'student-3', studentName: 'Сидоров Дмитрий Алексеевич' },
  { id: 'p-4', studentId: 'student-4', studentName: 'Козлова Анна Сергеевна' },
];

const RESULT_OPTIONS = ['1 место', '2 место', '3 место', 'Призёр', 'Участник', 'Медаль', 'Сертификат'];
...
export function UploadResults({ event, onBack, onPublish }: UploadResultsProps) {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-2xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-2xl font-bold text-foreground">Загрузка результатов</h2>
      ...
      <div className="bg-accent/10 border border-accent/20 rounded-lg px-5 py-3 flex gap-6 text-sm">
        <div><span className="text-muted-foreground">Тип: </span><span className="font-medium text-foreground">{event.type}</span></div>
        <div><span className="text-muted-foreground">Уровень: </span><span className="font-medium text-foreground">{event.level}</span></div>
        <div><span className="text-muted-foreground">Дата: </span><span className="font-medium text-foreground">{new Date(event.date).toLocaleDateString('ru-RU')}</span></div>
      </div>

      {/* Participants table */}
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        <div className="px-5 py-3 border-b border-border bg-secondary">
          <p className="text-sm font-semibold text-foreground">Таблица участников</p>
        </div>
        <table className="w-full">
          <thead>
            <tr className="border-b border-border">
              <th className="px-5 py-3 text-left text-sm font-semibold text-foreground">Имя</th>
            ...
            {MOCK_PARTICIPANTS.map(p => (
              <tr key={p.id} className="border-b border-border hover:bg-secondary/40 transition-colors">
                <td className="px-5 py-3 text-sm text-foreground">
                  {p.studentName}
                  {p.studentId === CURRENT_STUDENT.id && (
                    <span className="ml-2 text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">Вы</span>
                  )}
                </td>
